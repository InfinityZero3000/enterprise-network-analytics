...
```bash
cp .env.example .env
# Edit file .env với các API Keys của bạn (Gemini, Groq, OpenAI) nếu cần áp dụng Trợ lý Phân tích AI.
```

**3. Khởi phóng toàn bộ hạ tầng Dữ liệu và Backend API:**
Nền tảng chạy qua cụm multi-container của Docker Compose:
```bash
bash scripts/start.sh
# Hoặc khởi chạy thủ công bằng: docker compose up -d
```

**4. Khởi chạy giao diện Frontend Web UI:**
Tại một màn hình terminal tác biệt khác, duyệt tới thư mục `ui` và khởi động máy chủ Vite (Vite development server):
```bash
cd ui
npm install
npm run dev
